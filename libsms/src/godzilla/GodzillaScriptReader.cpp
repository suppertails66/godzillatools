...
GodzillaScriptReader::GodzillaScriptReader(
...
bool GodzillaScriptReader::operator()() {
  while (!src.eof()) {
    std::string line;
    src.getLine(line);
    ++lineNum;
...
void GodzillaScriptReader::loadThingy(const BlackT::TThingyTable& thingy__) {
  thingy = thingy__;
}
  
void GodzillaScriptReader::outputNextSymbol(TStream& ifs) {
  // literal value
  if ((ifs.remaining() >= 5)
      && (ifs.peek() == '<')) {
    int pos = ifs.tell();
...
//    std::cerr << std::dec << lineNum << " " << std::hex << result.id << " " << result.size << std::endl;
  ...
  throw TGenericException(T_SRCANDLINE,
                          "GodzillaScriptReader::outputNextSymbol()",
                          "Line "
...
void GodzillaScriptReader::flushActiveScript() {
  // write terminator
...
void GodzillaScriptReader::resetScriptBuffer() {
...
/*bool GodzillaScriptReader::checkSymbol(BlackT::TStream& ifs, std::string& symbol) {
...
void GodzillaScriptReader::processDirective(BlackT::TStream& ifs) {
  ...
  if (name.compare("LOADTABLE") == 0) {
    ...
    throw TGenericException(T_SRCANDLINE,
                            "GodzillaScriptReader::processDirective()",
                            "Line "
                              + TStringConversion::intToString(lineNum)
                              + ":\n  Unknown directive: "
                              + name);
...
void GodzillaScriptReader::processLoadTable(BlackT::TStream& ifs) {
  std::string tableName = TParse::matchString(ifs);
  TThingyTable table(tableName);
  loadThingy(table);
}

void GodzillaScriptReader::processStartMsg(BlackT::TStream& ifs) {
...
void GodzillaScriptReader::processEndMsg(BlackT::TStream& ifs) {
  flushActiveScript();
}

void GodzillaScriptReader::processIncBin(BlackT::TStream& ifs) {
...
void GodzillaScriptReader::processBreak(BlackT::TStream& ifs) {
  breakTriggered = true;
}
